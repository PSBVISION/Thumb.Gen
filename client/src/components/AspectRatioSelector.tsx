    ...
    <div className="space-y-3 dark">
      ...
      <div className="flex flex-wrap gap-2">
        {aspectRatios.map((ratio) => {
          ...
          return (
            <button key={ratio} type="button" onClick={()=>onChange(ratio)} className={`flex items-center gap-2  px-5 py-2.5 text-sm transition border border-white/10 rounded-md ${selected?'bg-white/10' :'hover:bg-white/5'}`}>
              {iconMap[ratio]}
